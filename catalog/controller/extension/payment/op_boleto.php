...
class ControllerExtensionPaymentOPBoleto extends Controller {
	...
	public function op_boleto_form() {
		...
		$this->load->model('checkout/order');
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_op_boleto_default_order_status_id'), '', false);
	...
	public function callback() {
		if (isset($this->request->post['order_number']) && !(empty($this->request->post['order_number']))) {
			...
			if($this->config->get('payment_op_boleto_logs') == 'True') {
				//记录浏览器返回日志
