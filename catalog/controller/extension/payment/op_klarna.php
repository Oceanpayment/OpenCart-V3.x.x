...
class ControllerExtensionPaymentOPKlarna extends Controller {
    ...
    public function index() {
        ...
        return $this->load->view('extension/payment/op_klarna', $data);
    }
    ...
    public function op_klarna_form() {
        ...
        $this->load->model('checkout/order');
        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_op_klarna_default_order_status_id'), '', false);
        ...
        if (!empty($order_info)) {
            ...
            //获取订单详情
            $productDetails = $this->getProductItems($product_info,$order_info['currency_code']);
            //获取消费者详情
            ...
            //提交网关
            $action = $this->config->get('payment_op_klarna_transaction');
            $data['action'] = $action;
            ...
            $validate_arr['terminal'] = $this->config->get('payment_op_klarna_terminal');
            $validate_arr['securecode'] = $this->config->get('payment_op_klarna_securecode');


            //商户号
            $account = $this->config->get('payment_op_klarna_account');
	    ...
	    $methods = 'Klarna';
            ...
            $title = mb_substr($productName,0,100);
            $sku = mb_substr($productSku,0,100);
            $price = round($getOrder_ET['sub_total']-$getOrder_ET['shipping']-$getOrder_ET['tax_total'],2);
            $price = $this->currency->format($price, $order_info['currency_code'], '', FALSE);
            $taxPrice = round(($getOrder_ET['sub_total']-$getOrder_ET['shipping'])*$tax_rate,2);
            $taxPrice = $this->currency->format($taxPrice, $order_info['currency_code'], '', FALSE);
            $shipping_price = $this->currency->format($getOrder_ET['shipping'], $order_info['currency_code'], '', FALSE);
            $tax_total_price = $this->currency->format($getOrder_ET['tax_total'], $order_info['currency_code'], '', FALSE);

            $itemList = [
                ["type"=>"1", "title"=>$title,"sku"=>$sku,"price"=>$price,"quantity"=>"1","total_amount"=>$price,"taxRate"=>$tax_rate,"taxPrice"=>$taxPrice,"image_url"=>"", "product_url"=>"","remark"=>""],
                ["type"=>"3", "title"=>"折扣","sku"=>$sku,"price"=>"0","quantity"=>"0","total_amount"=>"0","taxRate"=>"0","taxPrice"=>"0","image_url"=>"", "product_url"=>"","remark"=>""],
                ["type"=>"4", "title"=>"运费","sku"=>$sku,"price"=>$shipping_price,"quantity"=>"1","total_amount"=>$shipping_price,"taxRate"=>"1","taxPrice"=>$shipping_price,"image_url"=>"", "product_url"=>"","remark"=>""],
                ["type"=>"5", "title"=>"税费","sku"=>$sku,"price"=>$tax_total_price,"quantity"=>"1","total_amount"=>$tax_total_price,"taxRate"=>"1","taxPrice"=>$tax_total_price,"image_url"=>"", "product_url"=>"","remark"=>""],
            ];
            $itemList = json_encode($itemList,JSON_FORCE_OBJECT);
            ...
            if($this->config->get('payment_op_klarna_pay_mode') == 1){
                ...
                //跳转Redirect
                $this->response->setOutput($this->load->view('extension/payment/op_klarna_form', $data));
            }
    ...
    public function callback() {
        if (isset($this->request->post['order_number']) && !(empty($this->request->post['order_number']))) {
            ...
            if($this->config->get('payment_op_klarna_logs') == 'True'){
                //记录浏览器返回日志
            ...
            if($this->config->get('payment_op_klarna_transaction') == 'https://test-secure.oceanpayment.com/gateway/service/pay'){
                $message .= 'TEST ORDER-';
            ...
            if (strtoupper($local_signValue) == strtoupper($back_signValue)) {     //数据签名对比
                ...
                if($response_type == 0){
                    ...
                    if($ErrorCode == 20061){
                        ...
                        $this->response->setOutput($this->load->view('extension/payment/op_klarna_failure', $data));
                        ...
                        if ($payment_status == 1 ){
                            ...
                            $this->model_checkout_order->addOrderHistory($this->request->post['order_number'], $this->config->get('payment_op_klarna_success_order_status_id'), $message, true);

                            $data['continue'] = HTTPS_SERVER . 'index.php?route=checkout/success';
                            $this->response->setOutput($this->load->view('extension/payment/op_klarna_success', $data));
                            ...
                            $this->model_checkout_order->addOrderHistory($this->request->post['order_number'], $this->config->get('payment_op_klarna_pending_order_status_id'), $message, false);
                            ...
                            //交易失败
                            $this->model_checkout_order->addOrderHistory($this->request->post['order_number'], $this->config->get('payment_op_klarna_failed_order_status_id'), $message, false);
    ...
    public function notice() {
        ...
        if($this->xml_parser($xml_str)){
            ...
            //匹配终端号
            if($_REQUEST['terminal'] == $this->config->get('payment_op_klarna_terminal')){
                //普通终端号
                $securecode = $this->config->get('payment_op_klarna_securecode');
            }else{
                $securecode = '';
            ...
            if (strtoupper($local_signValue) == strtoupper($_REQUEST['signValue'])) {
                ...
                if($errorCode == 20061){
                    ...
                    if ($_REQUEST['payment_status'] == 1 ){
                        //交易成功
                        $this->model_checkout_order->addOrderHistory($_REQUEST['order_number'], $this->config->get('payment_op_klarna_success_order_status_id'), $message, false);
                    }elseif ($_REQUEST['payment_status'] == -1){
                        //交易待处理
                        //是否预授权交易
                        if($_REQUEST['payment_authType'] == 1){
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['order_number'], $this->config->get('payment_op_klarna_pending_order_status_id'), $message, false);
                    }else{
                        //交易失败
                        $this->model_checkout_order->addOrderHistory($_REQUEST['order_number'], $this->config->get('payment_op_klarna_failed_order_status_id'), $message, false);
                    }
                }
     ...
     * 获取订单详情
     */
    function getProductItems($AllItems,$currency_code){
        ...
        foreach ($AllItems as $item) {
            ...
            $productPrice[]         = $this->currency->format($item['price'], $currency_code, '', FALSE);
        }
