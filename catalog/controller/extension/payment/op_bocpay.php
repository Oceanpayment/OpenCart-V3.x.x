...
class ControllerExtensionPaymentOPBocpay extends Controller {
	...
	public function index() {
		...
		$data['action'] = 'index.php?route=extension/payment/op_bocpay/op_bocpay_form';
		
		
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		
		return $this->load->view('extension/payment/op_bocpay', $data);
	}

	
	public function op_bocpay_form() {
		
		$this->load->model('checkout/order');
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_op_bocpay_default_order_status_id'), '', false);

		
		//判断是否为空订单
		if (!empty($order_info)) {
			
			$this->load->model('extension/payment/op_bocpay');
			$product_info = $this->model_extension_payment_op_bocpay->getOrderProducts($this->session->data['order_id']);
			
			//获取订单详情
			$productDetails = $this->getProductItems($product_info);
			//获取消费者详情
			$customer_info = $this->model_extension_payment_op_bocpay->getCustomerDetails($order_info['customer_id']);
			...
			//提交网关
			$action = $this->config->get('payment_op_bocpay_transaction');
			$data['action'] = $action;
			...
			$validate_arr['terminal'] = $this->config->get('payment_op_bocpay_terminal');
			$validate_arr['securecode'] = $this->config->get('payment_op_bocpay_securecode');


			//商户号
			$account = $this->config->get('payment_op_bocpay_account');
			...
			//返回地址
			$backUrl = $base_url.'index.php?route=extension/payment/op_bocpay/callback';
			$data['backUrl'] = $backUrl;
			
			//服务器响应地址
			$noticeUrl = $base_url.'index.php?route=extension/payment/op_bocpay/notice';
			...
			//支付模式Pay Mode
			if($this->config->get('payment_op_bocpay_pay_mode') == 1){
				//内嵌Iframe
				$this->response->setOutput($this->load->view('extension/payment/op_bocpay_iframe', $data));
			}else{
				//跳转Redirect
				$this->response->setOutput($this->load->view('extension/payment/op_bocpay_form', $data));
			}
	...
	public function callback() {
		if (isset($this->request->post['order_number']) && !(empty($this->request->post['order_number']))) {
			$this->language->load('extension/payment/op_bocpay');
			...
			//匹配终端号
			if($terminal == $this->config->get('payment_op_bocpay_terminal')){
				//普通终端号
				$securecode = $this->config->get('payment_op_bocpay_securecode');
			}else{
				$securecode = '';
			}
			...
			if($this->config->get('payment_op_bocpay_transaction') == 'https://test-secure.oceanpayment.com/gateway/service/pay'){
				$message .= 'TEST ORDER-';
			...
			if (strtoupper($local_signValue) == strtoupper($back_signValue)) {     //数据签名对比
				...
				if($response_type == 0){		
					...
					if($ErrorCode == 20061){	 
						...
						$this->response->setOutput($this->load->view('extension/payment/op_bocpay_failure', $data));
						...
						if ($payment_status == 1 ){  
							...
							$this->model_checkout_order->addOrderHistory($this->request->post['order_number'], $this->config->get('payment_op_bocpay_success_order_status_id'), $message, true);
							
							$data['continue'] = HTTPS_SERVER . 'index.php?route=checkout/success';
							$this->response->setOutput($this->load->view('extension/payment/op_bocpay_success', $data));
							...
							$this->model_checkout_order->addOrderHistory($this->request->post['order_number'], $this->config->get('payment_op_bocpay_pending_order_status_id'), $message, false);
							...
							//交易失败
							$this->model_checkout_order->addOrderHistory($this->request->post['order_number'], $this->config->get('payment_op_bocpay_failed_order_status_id'), $message, false);
				...
				//数据签名对比失败
				$this->model_checkout_order->addOrderHistory($this->request->post['order_number'], $this->config->get('op_bocpay_failed_order_status_id'), $message, false);
	...
	public function notice() {
		...
		if($this->xml_parser($xml_str)){
			...
			//匹配终端号
			if($_REQUEST['terminal'] == $this->config->get('payment_op_bocpay_terminal')){
				//普通终端号
				$securecode = $this->config->get('payment_op_bocpay_securecode');
			}else{
				$securecode = '';
 			...
 			if (strtoupper($local_signValue) == strtoupper($_REQUEST['signValue'])) { 
				...
				if($errorCode == 20061){	 
					...
					if ($_REQUEST['payment_status'] == 1 ){
						//交易成功
						$this->model_checkout_order->addOrderHistory($_REQUEST['order_number'], $this->config->get('payment_op_bocpay_success_order_status_id'), $message, false);
					}elseif ($_REQUEST['payment_status'] == -1){
						//交易待处理
						//是否预授权交易
						if($_REQUEST['payment_authType'] == 1){
						...
						$this->model_checkout_order->addOrderHistory($_REQUEST['order_number'], $this->config->get('payment_op_bocpay_pending_order_status_id'), $message, false);
					}else{
						//交易失败
						$this->model_checkout_order->addOrderHistory($_REQUEST['order_number'], $this->config->get('payment_op_bocpay_failed_order_status_id'), $message, false);
					}
				}
